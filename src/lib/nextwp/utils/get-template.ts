...
export function getTemplate({
  ...
  if (
    ...
    const wpData = data as WpPage;
    if (wpData.type && !(wpData.type in templates)) {
      if (shouldLog) {
        debug.warn(
          `No templates found for type "${wpData.type}" on uri '${uri}'.\n Did you forget to add it to the templates object in src/templates/index?`
        );
      }
      return;
    }
    const templateName = getTemplateName(wpData.template || "default");
    const template = templates[wpData.type || ""][templateName];

    if (!template && shouldLog) {
      debug.warn(
        `Template "${templateName}" not found for type "${wpData.type}" on uri '${uri}'.\n Did you forget to add it to the templates object in src/templates/index?`
      );
      return;
    }
