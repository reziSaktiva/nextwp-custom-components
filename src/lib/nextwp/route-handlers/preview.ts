...
import { getSlugFromId } from "../api/get-single-item";
...
async function previewRouteHandler(
  request: Request,
  ...
  if (context.params.preview[0] === "preview") {
    ...
    if (id) {
      ...
      path = `/${slug}?secret=${process.env.NEXT_PREVIEW_SECRET}&preview=true`;
    }
    ...
    return new Response(null, {
      ...
      headers: {
        Location: `${path}${
          id ? "" : `?toolbar=${toolbarOption ? "true" : "false"}`
        }`,
      },
    });
